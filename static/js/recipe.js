...
function getRecipesAndFilters() {
    $.get('/recipes/', {'q': $('#searchbar').val(), 'initialized': 1, 'filter': getSelectedFilters()}, function (data) {
            ...
            prepareFilters(data.filters, data.selected_filters, data.filter_string, query);
        });
}
...
function prepareFilters(filters, selected_filters, filter_string, query) {
